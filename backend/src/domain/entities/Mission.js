...
class Mission {
  constructor(data = {}) {
    ...
    this.maximumAllowedTime = Number(data.maximumAllowedTime || data.duration || 90);
    this.aircraftSpeedKmph = Number(data.aircraftSpeedKmph || 900);
    this.requiredDamagePercentage = Math.min(
      100,
      Math.max(1, Number(data.requiredDamagePercentage || 100)),
    );
  ...
  assignAircraft(aircraftId) {
    if (!this.aircraftIds.includes(aircraftId)) this.aircraftIds.push(aircraftId);
  }
  assignPilot(pilotId) {
    if (!this.pilotIds.includes(pilotId)) this.pilotIds.push(pilotId);
  }
  assignGroundCrew(crewId) {
    if (!this.groundCrewIds.includes(crewId)) this.groundCrewIds.push(crewId);
  }
  ...
  applyStrikePlan(planningResult) {
    if (!planningResult?.success || !planningResult.bestPlan) return false;
  ...
  isInProgress() { return this.status === MissionStatus.IN_PROGRESS; }
  hasCompleted() { return this.isCompleted; }
