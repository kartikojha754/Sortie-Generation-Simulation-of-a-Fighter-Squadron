...
class SimulationService {
    ...
    runSimulation(input) {
        ...
        const missions = this.missionPlanner.createMissions(
            missionCount,
            scenario
        );
    ...
    registerHandlers(engine, context) {
        ...
        engine.registerHandler(EventType.TAKEOFF, (event) => {
            ...
            const runway = this.findById(
                context.squadron.runways,
                mission.runwayId
            );
            ...
            engine.scheduleEvent(
                EventFactory.createEvent({
                    ...
                    time:
                        event.time +
                        SimulationConstants.DEFAULT_SORTIE_DURATION,
                    entityId: mission.id,
                    payload: { missionId: mission.id }
                })
            );
        ...
        engine.registerHandler(EventType.LANDING, (event) => {
            ...
            if (aircraft) {
                const maintenanceResult =
                    this.maintenanceScheduler.scheduleMaintenance(
                        aircraft,
                        event.time
                    );

                context.maintenanceRecords.push(
                    maintenanceResult.maintenance
                );

                engine.scheduleEvent(
                    maintenanceResult.completionEvent
                );
            }
        });
